...
def main():
    ...
    api_key = "app-PYmSirQZfKrIE7mK0dtgBCww"  #小説04あらすじ詳細
    if not api_key:
    ...
    # 外部ファイル 'all_characters.md' からユーザープロンプト（1000文字程度）を読み込み
    all_characters_file = "all_characters.md"
    try:
        with open(all_characters_file, "r", encoding="utf-8") as f:
            all_characters = f.read()
    except Exception as e:
        return
    ...
    payload = {
        "inputs": {
            ...
            "all_characters": all_characters  # ワークフローの入力変数名に合わせてください
        },
