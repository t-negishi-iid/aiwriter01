 ...
 * 実行方法:
 * 1. インタラクティブモード:
 *    ts-node test_basic_setting.ts
 *
 * 2. 小説IDを指定して基本設定を取得:
 *    ts-node test_basic_setting.ts --id 1
 *
 * 3. 小説IDと基本設定作成用データIDを指定して基本設定を作成:
 *    ts-node test_basic_setting.ts --id 1 --action create --data-id 1
 *
 * 4. 自動テスト (エラー時に終了コード1を返す):
 *    ts-node test_basic_setting.ts --id 1 --test
 *
 * 5. バックエンドに直接アクセス:
 *    ts-node test_basic_setting.ts --id 1 --backend-direct
 *
 * 6. フロントエンドとバックエンドの結果を比較:
 *    ts-node test_basic_setting.ts --id 1 --compare
 */
...
function showHelp() {
  console.log(`
使用方法:
  ts-node test_basic_setting.ts [オプション]
